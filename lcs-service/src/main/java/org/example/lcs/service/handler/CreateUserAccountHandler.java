...
public class CreateUserAccountHandler {
    ...
    public UseResponse createUser(CreateUserAccountRequest userAccountRequest) {
        try {
    ...
    private UseResponse createResponse(UserAccount userAccount) {
        log.info("mapping response for the following user account ", userAccount);

        UseResponse userResponse = UserAccountMapper.toResponse(userAccount);
